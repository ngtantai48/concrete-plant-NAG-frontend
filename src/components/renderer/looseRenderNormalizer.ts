export const renderLoadingMarker = "\uE000_RENDER_LOADING_\uE000";
...
function parseTemplateAttributes(
  body: string
...
function canonicalRenderType(value: string): string | null {
  ...
  if (type === "pie_chart" || type === "pie" || type === "doughnut" || type === "donut")
    return "donut_chart";
...
function blockBase(type: string, data: Record<string, unknown>): Record<string, unknown> {
  ...
  const id =
    looseString(data.id) ?? `${type.replace(/_chart$/, "")}-${slugify(title)}-${currentHHmm()}`;
...
function toneFromLooseValues(tone: unknown, color: unknown): string | undefined {
  ...
  if (
    normalizedColor === "#007aff" ||
    normalizedColor === "#0a66e0" ||
    normalizedColor.includes("54, 162, 235")
  )
    return "blue";
  if (
    normalizedColor === "#34c759" ||
    normalizedColor === "#4caf50" ||
    normalizedColor.includes("75, 192, 192")
  )
    return "green";
  if (
    normalizedColor === "#ff9f0a" ||
    normalizedColor === "#ff9800" ||
    normalizedColor.includes("255, 205, 86")
  )
    return "amber";
  if (
    normalizedColor === "#ff3b30" ||
    normalizedColor === "#f44336" ||
    normalizedColor.includes("255, 99, 132")
  )
    return "red";
...
function ganttTone(tokens: string[]): GanttTone {
  ...
  if (/(crit|late|overdue|blocked|canceled|cancelled|da huy|loi|tre)/.test(normalized))
    return "red";
  if (/(done|complete|completed|hoan thanh)/.test(normalized)) return "green";
  if (/(active|running|transporting|moving|dang di chuyen|di chuyen|dang chay)/.test(normalized))
    return "blue";
  if (/(pending|wait|waiting|collecting|init|dang doi|cho|dang thu thap|thu thap)/.test(normalized))
    return "amber";
...
function normalizeSimpleChartBlock(
  type: "bar_chart" | "donut_chart",
...
function normalizeSeriesChartBlock(
  type: "line_chart" | "area_chart",
  data: Record<string, unknown>
): Record<string, unknown> {
...
function normalizeTableBlock(data: Record<string, unknown>): Record<string, unknown> {
  if (
    ...
    asRecordArray(data.columns).length > 0
  ) {
...
function uniqueSortedHours(values: number[]) {
  return [
    ...new Set(values.filter((hour) => Number.isFinite(hour) && hour >= 0 && hour <= 24)),
  ].sort((left, right) => left - right);
}

function ganttHours(data: Record<string, unknown>, chartData: Record<string, unknown>) {
  const explicit = Array.isArray(data.hours)
    ? data.hours
    : Array.isArray(chartData.hours)
      ? chartData.hours
      : [];
  const explicitHours = uniqueSortedHours(
    explicit.map(parseLooseHour).filter((hour): hour is number => hour !== null)
  );
  ...
  const labelHours = uniqueSortedHours(
    labels.map(parseLooseHour).filter((hour): hour is number => hour !== null)
  );
  if (labelHours.length > 0) return labelHours;

  const titleHours = collectHoursFromText(looseString(data.title) ?? "");
  if (titleHours.length >= 2)
    return expandHourRange(titleHours[0] ?? 0, titleHours[1] ?? titleHours[0] ?? 0);
  return [];
}

function ganttCellDescriptor(
  value: unknown
): { label: string; tone: GanttTone; tripId?: string } | null {
  if (value === null || value === undefined || value === false) return null;
  if (typeof value === "number" && value <= 0) return null;

  if (isRecord(value)) {
    const label = looseString(
      value.label ?? value.title ?? value.status ?? value.value ?? value.order ?? value.order_number
    );
    if (!label || ["0", "-", "—", "none", "null", "false"].includes(label.toLowerCase()))
      return null;
    return {
      label,
...
function normalizeExplicitGanttRows(rows: Array<Record<string, unknown>>): GanttOutputRow[] {
  return rows
    .map((row, rowIndex): GanttOutputRow | null => {
      ...
      const label =
        looseString(row.label ?? row.vehicle ?? row.vehicle_name ?? row.name) ??
        `Dòng ${rowIndex + 1}`;
...
function normalizeMatrixGanttRows(
  rawRows: unknown[],
  labels: string[],
  hours: number[]
): GanttOutputRow[] {
  ...
  const columns =
    hourColumns.length > 0 ? hourColumns : hours.map((hour, index) => ({ hour, index }));

  return rawRows
    .map((rawRow, rowIndex): GanttOutputRow | null => {
      const rowRecord = isRecord(rawRow) ? rawRow : null;
      const values =
        rowRecord && Array.isArray(rowRecord.data)
          ? rowRecord.data
      ...
      const label =
        (rowRecord &&
          looseString(
            rowRecord.label ?? rowRecord.vehicle ?? rowRecord.vehicle_name ?? rowRecord.name
          )) ??
        (Array.isArray(rawRow) && columns[0]?.index !== 0 ? looseString(rawRow[0]) : undefined) ??
        `Dòng ${rowIndex + 1}`;
      const sub = rowRecord
        ? looseString(rowRecord.sub ?? rowRecord.vehicle_license_plate ?? rowRecord.license_plate)
        : undefined;
      const valueOffset =
        !rowRecord &&
        Array.isArray(rawRow) &&
...
function normalizeGanttBlock(data: Record<string, unknown>): Record<string, unknown> {
  ...
  let rows =
    explicitRows.length > 0 && explicitRows.some((row) => Array.isArray(row.blocks))
...
function normalizeStandaloneBlock(type: string, value: unknown): Record<string, unknown> | null {
  ...
  if (canonical === "bar_chart" || canonical === "donut_chart")
    return normalizeSimpleChartBlock(canonical, data);
  if (canonical === "line_chart" || canonical === "area_chart")
...
function convertTemplateChart(name: string, attributes: Record<string, string>): string | null {
  ...
  const values = asNumberArray(
    templateArray(attributes.values ?? attributes.value ?? attributes.y)
  );
...
function convertTemplateRender(body: string): string | null {
  ...
  if (parsed.name === "chart")
    return convertTemplateChart(parsed.attributes.type ?? parsed.name, parsed.attributes);
  return convertTemplateChart(parsed.name, parsed.attributes);
}

function normalizeTemplateRenderBlocks(text: string): string {
  if (
    !/\{\{\s*(?:chart|bar|bar_chart|donut|donut_chart|pie|pie_chart|doughnut|line|line_chart|area|area_chart|table)\b/i.test(
      text
    )
  ) {
    return text;
...
function parseLooseChartObject(value: string): unknown | null {
  ...
  for (const candidate of candidates) {
    ...
    const repaired = candidate
      .replace(
        /([{,]\s*)(labels|datasets|label|data|backgroundColor|borderColor|borderWidth|options|plugins|title|text|value)\s*:/g,
        '$1"$2":'
      )
      .replace(
        /'([^'\\]*(?:\\.[^'\\]*)*)'/g,
...
function parseYamlArrayKey(
  lines: string[],
  key: string,
  startIndex = 0
): { values: unknown[]; lineIndex: number } | null {
...
function parseYamlDatasets(lines: string[]): Array<Record<string, unknown>> {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (borderMatch) {
      ...
      current.borderColor = rest.startsWith("[")
        ? (parseInlineArrayLiteral(rest) ?? undefined)
        : unquoteYamlValue(rest);
    }
  }
...
function convertXmlChartTag(tag: string): string | null {
  ...
  const title =
    ...
    inferChartTitle(
      type === "bar_chart" || type === "bar"
        ? "bar"
        : type === "line_chart" || type === "line"
          ? "line"
          : type === "area_chart" || type === "area"
            ? "area"
            : "donut",
      labels
    );
...
function convertXmlChartElement(openTag: string, body: string): string | null {
  ...
  try {
    ...
    const chart =
      isRecord(parsedBody) && typeof parsedBody.type === "string" && isRecord(parsedBody.data)
        ? parsedBody
        : {
            type: chartType,
            data: parsedBody,
    ...
    return convertXmlChartTag(
      `<chart type="${chartType}" title="${title}" data="${decodedBody}" />`
    );
  } catch {
    const data = parseLooseKeyValueData(decodedBody);
    if (data.length === 0) return null;
...
function normalizeXmlChartTags(text: string): string {
  ...
  return text
    ...
    .replace(
      /(?:<|&lt;)chart\b[\s\S]*?(?:>|&gt;)([\s\S]*?)(?:<|&lt;)\/chart(?:>|&gt;)/gi,
      (tag, body: string) => convertXmlChartElement(tag, body) ?? tag
    );
}
...
function mermaidTone(tokens: string[]): "blue" | "green" | "amber" | "red" | "purple" {
  ...
  if (
    joined.includes("crit") ||
    joined.includes("late") ||
    joined.includes("overdue") ||
  ...
  if (
    ...
    joined.includes("hoàn thành")
  )
    return "green";
  if (
    joined.includes("active") ||
    joined.includes("running") ||
    joined.includes("transporting") ||
    joined.includes("moving") ||
    joined.includes("di chuyển")
  )
    return "blue";
  if (
    joined.includes("pending") ||
    joined.includes("wait") ||
    joined.includes("waiting") ||
    joined.includes("đang đợi") ||
    joined.includes("collecting")
  )
    return "amber";
  return "purple";
}
...
function convertMermaidGantt(body: string): string | null {
  ...
  for (const line of lines.slice(1)) {
    ...
    const duration = tokens
      .map(parseMermaidDuration)
      .find((value): value is number => value !== null);
    const end = Math.max(start + 0.25, times[1] ?? (duration ? start + duration : start + 1));
    const blockLabel =
      tokens
        .filter((token) => parseMermaidHour(token) === null && parseMermaidDuration(token) === null)
        .join(" · ") ||
      currentSection ||
      taskLabel;
...
function normalizeMermaidGanttBlocks(text: string): string {
  ...
  return text
    .replace(
      /`{3,}\s*(?:mermaid|gantt)\s*\n([\s\S]*?)`{3,}/gi,
      (fence, body: string) => convertMermaidGantt(body) ?? fence
    )
    .replace(
      /`{3,}\s*\n(\s*gantt[\s\S]*?)`{3,}/gi,
      (fence, body: string) => convertMermaidGantt(body) ?? fence
...
function normalizeYamlChartFences(text: string): string {
  ...
  return text.replace(
    /`{3,}\s*(?:ya?ml|chart)?\s*\n([\s\S]*?)`{3,}/gi,
    (fence, body: string) => convertYamlChartBlock(body) ?? fence
  );
}
...
function convertChartJsBlock(chart: unknown): string | null {
    ...
    if (datasets.length > 1) {
      const columns = [
        ...
        ...datasets.map((dataset, index) => ({
          key: tableKey(
            typeof dataset.label === "string" ? dataset.label : `Chỉ số ${index + 1}`,
            index
          ),
  ...
  if (chartType === "line" || chartType === "area") {
    const title = chartJsTitle(
      chart,
      inferChartTitle(chartType === "area" ? "area" : "line", labels, datasetLabel(datasets[0]))
    );
...
function isChartLikeCodeFence(language: string | undefined, body: string): boolean {
  ...
  return /^(?:type\s*:\s*(?:bar|bar_chart|pie|pie_chart|donut|donut_chart|doughnut|line|line_chart|area|area_chart)|gantt\b|:::render\b|{\s*["']?type["']?\s*:\s*["'](?:bar|doughnut|donut|pie|line|area|bar_chart|donut_chart|line_chart|area_chart|gantt|kpi_grid|table|timeline|map_view|image|file|alert|action_proposal|source_chips|followups)|{\s*["'](?:kpi_grid|line_chart|bar_chart|donut_chart|area_chart|gantt|timeline|table|map_view|image|file|alert|action_proposal|markdown|source_chips|followups|pie_chart)["']\s*:|<chart\b)/i.test(
    decodedBody
  );
}

export function hideUnresolvedRenderBuffers(text: string): string {
  return text
    .replace(
      /`{3,}\s*([a-zA-Z_-]+)?[^\n]*\n([\s\S]*?)`{3,}/g,
      (fence, language: string | undefined, body: string) =>
        isChartLikeCodeFence(language, body) ? renderLoadingMarker : fence
    )
    ...
    .replace(
      /\{\{\s*(?:chart|bar|bar_chart|donut|donut_chart|pie|pie_chart|doughnut|line|line_chart|area|area_chart|table)\b[\s\S]*?\}\}/gi,
      renderLoadingMarker
    );
}
...
export function normalizeLooseRenderBlocks(text: string): string {
    ...
    normalizeYamlChartFences(
      normalizeMermaidGanttBlocks(
        normalizeTemplateRenderBlocks(
          normalizeXmlChartTags(
            normalizeStandaloneRenderJsonBlocks(
              normalizeChartJsRenderBlocks(unwrapRenderFenceWrappers(text))
            )
          )
        )
      )
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const headingMatch = /^Render Block:\s*(.+?)(?:\s*\((?:Donut Chart|donut_chart)\))?\s*$/i.exec(
      previousLine
    );
...
export function normalizeRenderBlockData(data: unknown): unknown {
...
function findPendingStandaloneRenderJsonStart(text: string): number {
  ...
  for (const pattern of patterns) {
    ...
    let match: RegExpExecArray | null;
    while ((match = pattern.exec(text)) !== null) {
      if (!isInsideRenderFence(text, match.index) && findJsonObjectEnd(text, match.index) < 0)
        pendingStart = match.index;
    }
  }
...
function findPendingTemplateRenderStart(text: string): number {
  const pattern =
    /\{\{\s*(?:chart|bar|bar_chart|donut|donut_chart|pie|pie_chart|doughnut|line|line_chart|area|area_chart|table)\b/gi;
  let match: RegExpExecArray | null;
  let pendingStart = -1;
...
export function findPendingRenderStart(text: string): number {
  const starts = [
    text.indexOf(":::render"),
