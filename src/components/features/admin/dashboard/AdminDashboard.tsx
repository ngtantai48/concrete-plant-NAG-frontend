  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans tracking-tight">
      <div className="p-4 md:p-8 max-w-[1800px] mx-auto">
        ...
        <div className="mb-8 border-b-4 border-slate-900 bg-white p-6 shadow-sm">
        ...
        <Tabs
          ...
          items={[
                    ...
                    {/* Left: Vehicle Availability Lists */}
                     <div className="lg:col-span-3 space-y-6">
                        <div className="flex h-[400px] flex-col border border-slate-300 bg-white">
                          <div className="flex items-center justify-between border-b border-slate-300 bg-slate-50 px-4 py-3 text-sm font-bold uppercase tracking-[0.18em] text-slate-900">
                            <span>{t('readyVehiclesPanel')}</span>
                            <span className="border border-slate-300 bg-white px-3 py-1 text-xs font-bold text-slate-900">{readyVehicles.length}</span>
                          </div>
                         <div className="overflow-y-auto p-0 flex-1">
                           {readyVehicles.length === 0 ? (
                              <div className="p-4 text-sm font-bold text-slate-400">{t('noReadyVehicles')}</div>
                           ) : (
                             <ul className="divide-y divide-slate-200">
                               {readyVehicles.map((v) => (
                                  <li key={v.vehicle_id} className="flex items-center gap-3 p-4 hover:bg-slate-50">
                                    ...
                                    <span className="text-lg font-black text-slate-900">{v.vehicle_license_plate}</span>
                                  </li>
                               ))}
                             </ul>
                           )}
                        ...
                        <div className="flex h-[280px] flex-col border border-slate-300 bg-white">
                          <div className="flex items-center justify-between border-b border-slate-300 bg-slate-50 px-4 py-3 text-sm font-bold uppercase tracking-[0.18em] text-slate-900">
                            <span>{t('stoppedMaintenance')}</span>
                            <span className="border border-slate-300 bg-white px-3 py-1 text-xs font-bold text-slate-900">{canceledOrders.length}</span>
                          </div>
                         <div className="flex-1 overflow-y-auto bg-white p-0">
                            {canceledOrders.length === 0 ? (
                               <div className="p-4 text-sm font-bold text-slate-400">{t('empty')}</div>
                            ) : (
                              <ul className="divide-y divide-slate-200">
                                {canceledOrders.map((o) => (
                                  <li key={o.order_id} className="flex items-center justify-between p-4">
                                    <span className="font-black text-slate-900">{o.vehicles?.vehicle_license_plate || `#${o.order_id}`}</span>
                                      <span className="border border-amber-500 bg-amber-50 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-amber-600">{t('canceled')}</span>
                                   </li>
                                ))}
                              </ul>
                          )}
                     ...
                     <div className="lg:col-span-3 h-[704px]">
                        <div className="flex h-full flex-col border border-slate-300 bg-white">
                          <div className="flex items-center justify-between border-b border-slate-300 bg-slate-50 px-4 py-3 text-sm font-bold uppercase tracking-[0.18em] text-slate-900">
                            ...
                            <span className="border border-slate-300 bg-white px-3 py-1 text-xs font-bold text-slate-900">{outsideOrders.length}</span>
                          </div>
                         <div className="flex-1 overflow-y-auto">
                            {outsideOrders.length === 0 ? (
                              <div className="flex items-center justify-center h-full">
                                 <div className="border border-slate-300 bg-slate-50 px-6 py-4">
                                   <p className="text-sm font-bold text-slate-500 text-center">{t('waitingSignal')}</p>
                                </div>
                              </div>
                            ) : (
                              <ul className="flex flex-col gap-3 p-3">
                                {outsideOrders.map((o) => (
                                    <li key={o.order_id} className="border border-slate-300 bg-white p-4 transition-colors hover:border-slate-900">
                                     <div className="flex justify-between items-end">
                                       <span className="text-lg font-black text-slate-900">{o.vehicles?.vehicle_license_plate || `#${o.order_id}`}</span>
                                     </div>
                                    <div className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-slate-400">{o.stations?.station_name || t('unassigned')}</div>
                                  </li>
                                ))}
                             </ul>
                          )}
            ...
            },
            {
              ...
              children: (
                 <div className="mt-6 h-[700px] border border-slate-300 bg-white p-2">
                   <StationMap
        ...
        />
         ...
         <div className="mb-20 mt-8 flex flex-col justify-between gap-4 border-t-2 border-slate-900 pt-4 md:flex-row md:items-baseline whitespace-nowrap">
            <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-[0.18em] text-slate-900">
              <span className="flex items-center gap-2 border border-slate-300 bg-white px-3 py-1 text-slate-900">
                <div className="h-1.5 w-1.5 bg-emerald-500 animate-ping" />
                {t('systemSignal')}
              </span>
