...
const toVehicleId = (value: any): number | null => {
...
export default function TankStatusTab({ tanks, loading, useVTracking, setUseVTracking, vehicles, selectedVehicleId, onRequestRefresh, onSelectedVehicleChange }: { tanks: VehicleTankStatus[], loading: boolean, useVTracking: boolean, setUseVTracking: (v: boolean) => void, vehicles?: Vehicle[], selectedVehicleId?: number, onRequestRefresh?: () => void, onSelectedVehicleChange?: (vehicleId: number) => void }) {
  ...
  const lastSyncIdRef = useRef<number | null>(normalizedSelectedVehicleId);
  ...
  useEffect(() => {
    if (!useVTracking || !tanks?.length) return;
    const selectedVehicleId = toVehicleId(baseTank?.vehicle_id);
    const prefetchCandidates = tanks
      .map((tank) => ({
        id: toVehicleId(tank.vehicle_id),
        openingAt: tank.configured_opening_fuel_at ?? null,
      }))
      .filter((item): item is { id: number; openingAt: string | null } => Boolean(item.id))
    ...
    const timer = setTimeout(async () => {
      for (const candidate of prefetchCandidates) {
        if (cancelled) return;
        if (prefetchingRef.current.has(candidate.id)) continue;
        const cached = detailCacheRef.current.get(candidate.id);
        const isFresh =
          Boolean(cached?.data) &&
          cached?.openingAt === candidate.openingAt &&
          (Date.now() - (cached?.fetchedAt || 0)) <= VTRACKING_OVERVIEW_CACHE_TTL_MS;
        if (isFresh) continue;
        prefetchingRef.current.add(candidate.id);
        try {
          const params: any = {
          ...
          if (candidate.openingAt) {
            params.from = dayjs(candidate.openingAt).startOf("day").format("YYYY-MM-DD HH:mm:ss");
          }
          const res = await fuelApi.getTankStatus(params);
          if (cancelled) return;
          const item = res.data?.items?.[0] || (Array.isArray(res.data) ? res.data[0] : null);
          if (!item) continue;
          detailCacheRef.current.set(candidate.id, {
            data: item,
            fetchedAt: Date.now(),
          ...
          prefetchingRef.current.delete(candidate.id);
        }
      }
    }, 250);
  ...
  }, [useVTracking, tanksFingerprint, baseTank?.vehicle_id, snapshotAt]);
  ...
  }, [baseTank?.vehicle_id, refreshTick, snapshotAt, useVTracking, isOverviewReady]);
  ...
  }, [activeTank?.vehicle_id, activeTank?.current_fuel_liters, refreshTick, snapshotAt, useVTracking, isOverviewReady]);
  ...
  }, [activeTank?.vehicle_id, refreshTick, useVTracking, isOverviewReady]);
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            {activeTab === 'Tổng quan' ? (
              ...
              <div className="h-full -mx-6 -mb-6">
                <FuelEventsTab from={fromDateTime || ''} to={toDateTime || ''} vehicle_id={toVehicleId(activeTank.vehicle_id) ?? undefined} vehicles={vehicles || []} onMutated={triggerDataRefresh} />
              </div>
            ) : activeTab === 'Vị trí' ? (
              ...
              <div className="h-full -mx-6 -mb-6">
                <FuelMetricsTab from={fromDateTime || ''} to={toDateTime || ''} vehicle_id={toVehicleId(activeTank.vehicle_id) ?? undefined} />
              </div>
            ) : activeTab === 'Cấu hình' ? (
              <div className="h-full -mx-6 -mb-6">
