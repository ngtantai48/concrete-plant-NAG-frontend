  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: VehicleType) => (
        <Space size="middle">
          {hasActionAccess(SIDEBAR.VEHICLE_TYPES, PERMISSIONS.VEHICLE_TYPES.UPDATE) && (
          ...
          {hasActionAccess(SIDEBAR.VEHICLE_TYPES, PERMISSIONS.VEHICLE_TYPES.DELETE) && (
            <Popconfirm
      ...
      <div className="m-10 bg-white rounded-lg shadow-sm border border-slate-200 animate-fade-in overflow-hidden">
        <div className="p-6 md:p-8 border-b-2 border-slate-100 flex items-start justify-between gap-6 flex-wrap bg-slate-50/50">
          ...
          <div className="flex gap-3 mt-2 sm:mt-0 flex-wrap">
            {hasActionAccess(SIDEBAR.VEHICLE_TYPES, PERMISSIONS.VEHICLE_TYPES.CREATE) && (
